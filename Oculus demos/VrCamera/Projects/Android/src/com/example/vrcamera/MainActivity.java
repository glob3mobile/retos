...
public class MainActivity extends VrActivity implements TextureView.SurfaceTextureListener {
	...
	public void onSurfaceTextureAvailable(SurfaceTexture surface, int width,
		...
		Log.e("SFACE TXTURE INIT SIZE", "W: "+width+",H: "+height);
		initCamera();
		try {
	...
	private void resetEyes(){
		...
		leftEye = new VrSurfaceView(this);
		rightEye = new VrSurfaceView(this);
		LinearLayout.LayoutParams params2 = new LinearLayout.LayoutParams(0, ViewGroup.LayoutParams.MATCH_PARENT,1);
		leftEye.setLayoutParams(params2);
		rightEye.setLayoutParams(params2);
		leftEye.setWillNotDraw(false);
		rightEye.setWillNotDraw(false);
	...
	private void initGlobe(){
		...
		builder.addCameraConstraint(new ICameraConstrainer(){
			...
			public boolean onCameraChange(Planet planet, org.glob3.mobile.generated.Camera previousCamera,
				...
				//labelBuilder.setText("P: " + nextCamera.getGeodeticPosition()._latitude._degrees + ", " + nextCamera.getGeodeticPosition()._longitude._degrees + " H: " + nextCamera.getGeodeticPosition()._height);
		...
		//Black magic to make stylish things, just for a photo
		//widget.getNextCamera().setGeodeticPosition(Geodetic3D.fromDegrees(0,0,350));
	}
	...
	private void addMarkers() {
    	...
    	/*Geodetic2D rotondaPos = Geodetic2D.fromDegrees(28.072615, -15.451247);
    	double rotondaHgt = elevationData.getElevationAt(rotondaPos);
    	Mark mark = new Mark(
				new URL("file:///bolita.png",false),
				new Geodetic3D(rotondaPos,rotondaHgt),
				AltitudeMode.ABSOLUTE,
				1000000);

    	Geodetic2D rotonda2Pos = Geodetic2D.fromDegrees(28.073040, -15.452103);
    	double rotonda2Hgt = elevationData.getElevationAt(rotonda2Pos);
    	Mark mark2 = new Mark(
				new URL("file:///bolita.png",false),
				new Geodetic3D(rotonda2Pos,rotonda2Hgt),
				AltitudeMode.ABSOLUTE,
				1000000);

		marksRenderer.addMark(mark);
		marksRenderer.addMark(mark2);*/

	}

	private void addSomeShapes(){
		// Añadiremos un círculo random en la rotonda de Informática
		// Añadiremos una pelota en el mar a la altura de San Telmo
		// Añadiremos una caja enorme a la altura de mi casa
		...
		//La bola podría o no estar contenida en gcSector
		...
		//Los otros dos están contenidos de seguro!
		...
		//shapesRenderer.addShape(theBox);
		shapesRenderer.addShape(theBall2);
		//shapesRenderer.addShape(theCircle);
		//shapesRenderer.addShape(theBox2);
		//shapesRenderer.addShape(theBox3);
		shapesRenderer.addShape(theBall);
	...
	public class MyElevationDataListener implements IElevationDataListener {
		...
		public void onData(Sector sector, Vector2I extent, ElevationData elevData){
			...
			if (widget != null){
				//Black magic to stop rendering globe:
